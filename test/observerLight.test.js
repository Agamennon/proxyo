...
describe('nx-observe', () => {
  ...
  describe('reaction', () => {
    it('should call reaction', () => {
      let dummy
      let reacted = null
      const observable = observer.observable({counter: 0})
      observer.reaction(() => dummy = observable.counter,null,()=>{
        reacted = 'reacted'
        observable.counter = observable.counter + 1
      })


      return Promise.resolve()
          .then(() => expect(dummy).to.equal(0))
          .then(() => expect(reacted).to.equal(null))
          .then(() => observable.counter = 7)
          .then(() => expect(dummy).to.equal(0))
          .then(() => expect(reacted).to.equal('reacted'))
          .then(() => expect(observable.counter).to.equal(8))
    })
  })
